<div class="row align-items-center">
    <div class="col-auto">
        ...
        @if($post->isLiked())
           ...
           <form action="{{route('like.destroy', $post->id)}}" method="post">
              @csrf
              @method('DELETE')
              <button type="submit" class="border-8 bg-transparent shadow-none p-0">
    ...
    <div class="col-auto px-0">
        {{-- no.of likes --}}
        <button class="dropdown-item" data-bs-toggle='modal' data-bs-target="#like-post{{$post->id}}">
            {{$post->likes->count()}}
        </button>
    </div>
    @include('user.posts.contents.modals.like')
